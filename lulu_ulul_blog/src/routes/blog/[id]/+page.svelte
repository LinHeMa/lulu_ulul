...
<svelte:head>
  {#if post}
    <title>{post.title} | LinHeMa de Blog</title>
    <meta name="description" content={post.body.substring(0, 160)} />
  {:else}
    <title>Blog Post | LinHeMa de Blog</title>
  {/if}
</svelte:head>
...
<div>
  ...
  {:else if error}
    <div class="py-12 text-center">
      ...
      <p class="text-gray-500 dark:text-gray-400 mt-2">Unable to load blog post. Please try again later.</p>
      <div class="mt-6">
    ...
    <article class="prose dark:prose-invert lg:prose-lg max-w-none">
        ...
        {#if post.tags.length > 0}
          <div class="flex flex-wrap gap-2 mb-6">
            {#each post.tags as tag}
              <a href={`/tags/${tag}`} class="inline-block px-3 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600">
                {tag}
              </a>
            {/each}
          </div>
      ...
      <div class="mt-12 pt-6 border-t border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center mb-6">
          <a href="/" class="inline-block text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
            &larr; Back to all posts
          </a>
          <a href={post.html_url} target="_blank" rel="noopener noreferrer" class="inline-block text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
            View on GitHub
          </a>
        </div>
      </div>
